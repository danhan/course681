...
/*			System.out.println(rowRange[0]+":"+rowRange[1]);
			for(int i=0;i<c.length;i++){
				System.out.print(c[i]+";");
			}
			System.out.println();	*/		
			// the version here is harded coded, because i cannot get how many objects in one cell now
			rScanner = this.hbaseUtil.getResultSet(rowRange,null, this.familyName,c,100);
				...
				for(byte[] family:resultMap.keySet()){
					...
					for(byte[] col:columns.keySet()){
						...
						for(Long version:values.keySet()){
							...
							if(distance <= radius){								
								//System.out.println("row=>"+Bytes.toString(r.getRow()) + ";colum=>"+Bytes.toString(col)+";version=>"+version+ ";station=>"+station.getId()+";distance=>"+distance);							
								accepted++;
	...
	public void scanQueryPoint(double latitude, double longitude) {
		...
		try{
			...
			// the version here is harded coded, because i cannot get how many objects in one cell now
			rScanner = this.hbaseUtil.getResultSet(rowRange,null, this.familyName,new String[]{match_box.getColumn()},100);
			BixiReader reader = new BixiReader();
			int count = 0;			
